...
function monthGrid(cal) {
  ...
  for (const wk of monthWeeks(cal.year, cal.month)) {
    for (const d of wk) {
      ...
      if (d === cal.today) { cells += `<div class="cell"><span class="today">${d}</span></div>`; continue; }
      const ev = cal.eventDays.includes(d);
      cells += `<div class="cell"><span>${d}</span><i class="dot ${ev ? "" : "off"}"></i></div>`;
    }
  }
...
export function html(data) {
  ...
  .cell .today{background:${RED};color:#fff;width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:600;flex-shrink:0;line-height:1}
