...
function face(cssName, weight, file) {
  const b64 = fs.readFileSync(path.resolve(file)).toString("base64");
  return `@font-face{font-family:'${cssName}';font-style:normal;font-weight:${weight};src:url(data:font/woff;base64,${b64}) format('woff');}`;
}
function fontset(dir, prefix, cssName, weights) {
  return weights.map((w) => face(cssName, w, `${dir}/${prefix}-latin-${w}-normal.woff`)).join("\n");
}
export const FONT_CSS = fontset("node_modules/@fontsource/inter/files", "inter", "Inter", [400, 500, 600, 700, 800, 900]);
export const SERIF_CSS = fontset("node_modules/@fontsource/playfair-display/files", "playfair-display", "Playfair Display", [400, 700, 800, 900]);
export const MONO_CSS = fontset("node_modules/@fontsource/ibm-plex-mono/files", "ibm-plex-mono", "IBM Plex Mono", [400, 500, 600, 700]);

// ---- placeholder/real photo -------------------------------------------------
// Reads assets/photo.{jpg,jpeg,png,webp} if present and returns a data URI so the
// HTML stays self-contained. Returns null if no photo file exists (styles then
// fall back to a drawn placeholder scene).
export function photoDataUri() {
  for (const ext of ["jpg", "jpeg", "png", "webp"]) {
    const abs = path.resolve(`assets/photo.${ext}`);
...
export function icon(kind, size) {
  return `<span class="wic" style="width:${size}px;height:${size}px">${weatherSvg(kind)}</span>`;
}
export const ICON_CSS = `.wic{display:inline-block;line-height:0}.wic svg{width:100%;height:100%;display:block}`;
...
export function pageShell({ title = "dashboard", css, body, bg = "#FFFFFF", fonts = "" }) {
